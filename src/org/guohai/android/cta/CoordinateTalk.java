...
public class CoordinateTalk extends Activity {
	...
	private Button.OnClickListener btnTestBMI = new Button.OnClickListener() {
		public void onClick(View v) {
			...
			for (int i = 0; i < count; i++) {
				a += listNbInfo.get(i).getCid() + ","
						+ listNbInfo.get(i).getLac() + ","
						+ listNbInfo.get(i).getRssi() + "|";// 取邻居小区号
			}
	...
	private Button.OnClickListener btnWhereBMI = new Button.OnClickListener() {
		public void onClick(View v) {
			...
			if (parseInfo.Latitude == 0 || parseInfo.Longitude == 0) {
				Toast.makeText(CoordinateTalk.this, "定位中..", Toast.LENGTH_SHORT)
						.show();
				return;
			}
	...
	private Button.OnClickListener btnReferBMI = new Button.OnClickListener() {
		public void onClick(View v) {
			...
			if (message.Note.length() == 0) {
				Toast.makeText(CoordinateTalk.this, "发送内容不能为空",
			...
			ResultInfo result = httpRest.AddMessage(message,
					getApplicationContext());
			Log.i(TAG, "[" + result.State + result.Message + "]");
			if (0 <= result.State) {
				Toast.makeText(CoordinateTalk.this, result.Message,
						Toast.LENGTH_SHORT).show();
				editMessage.setText("");
			} else {
				Toast.makeText(CoordinateTalk.this, result.Message,
	...
	private void inita() {
		...
		SimpleAdapter mSchedule = new SimpleAdapter(this, mylist,
				R.layout.list_local_message_item, new String[] { "ItemName",
						"ItemMessage" }, new int[] { R.id.ItemName,
						R.id.ItemMessage });
